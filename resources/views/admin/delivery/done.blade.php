...
;"> <a href="#" style="color:white;">Экселээс
                        ...
                        <button type="button" class="btn btn-primary"> <a
                                href="https://e-chuchu.mn/dist/Excel_blank_delivery.xlsx" style="color:white;">Экселийн
                                Загвар татаж
                                авах</a></button>
                        <div class="row">
                            <div class="row">
                                ...
                                <div class="form-group myform">
                                    ...
                                    <select id="filterByCustomerType" class="form-control inputStatus9">
                                        ...
                                        <option value="6">10+ бөөний</option>
                                    </select>
                                </div>
                                <div class="form-group myform">
                            ...
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <!-- /.card-header -->
                                    <div class="card-body table-wrapper table-responsive p-0 table-container ">
                                        <table class="table table-hover text-nowrap small" id="datatable"
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center whitespace-nowrap table-danger">Хүлээн авагчийн
                                                        нэр
                                                    </th>
                                                    ...
                                                    <th class="text-center whitespace-nowrap table-danger">Барааны тооцоо
                                                    </th>
                                                    <th class="text-center whitespace-nowrap table-danger">Хүлээн авагчийн
                                                        гарын
                                                        үсэг</th>
...
;color:white;">Устгах</button>
                                ...
                                <div id="reorderModal" class="modal">
                                    <div class="modal-content text-center"
                                        style="width:400px !important;height:300px !important;margin-top:200px;">
                                        <div class="modal-header">
                                            <h4 class="modal-title">Дахин захиалах</h4>
                                        </div>
                                        <div class="modal-body">
                                            <div class="form-group">
                                                <label for="reorder_number">Тоо ширхэг:</label>
                                                <input type="number" class="form-control" id="reorder_number" name="reorder_number" value="" min="1">
                                                <input type="hidden" id="reorder_delivery_id" value="">
                                            </div>
                                        </div>
                                        <div class="modal-footer justify-content-between">
                                            <button type="button" class="btn btn-default closing"
                                            ...
                                            <button type="button"
                                                class="btn btn-primary btn_reorder_confirm">Дахин захиалах</button>
                                        </div>
                                    </div>
                                </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function($j) {
            ...
            const loadDeliveryDataTable = (status, region, driver, customer, not_2, not_1, not_6, not_10,
            ...
            loadDeliveryDataTable(status, region, driver, customer, not_2, not_1, not_6, not_10, not_100,
                merchant, type, estimated, start_date, end_date);
            ...
            loadDeliveryDataTable(status, region, driver, customer, not_2, not_1, not_6, not_10, not_100, merchant,
                type, estimated);
            $('#filterByCustomerType').change(function() {
                ...
                rows_selected.length = 0;
                loadDeliveryDataTable(status, region, driver, customer, not_2, not_1, not_6, not_10,
                    not_100, merchant, type, estimated);
            });

            loadDeliveryDataTable(status, region, driver, customer, not_2, not_1, not_6, not_10, not_100, merchant,
                type, estimated, start_date, end_date);
    ...
    updateCount();
            ...
            // Handle Re-order button click for individual row
            $(document).on('click', '.btn-reorder-row', function() {
                var deliveryId = $(this).data('id');
                var currentNumber = $(this).data('number') || '';
                
                $('#reorder_delivery_id').val(deliveryId);
                $('#reorder_number').val(currentNumber);
                $('#reorderModal').attr('style', 'display:block');
            });

            // Handle Re-order confirmation
            $(document).on('click', '.btn_reorder_confirm', function() {
                var deliveryId = $('#reorder_delivery_id').val();
                var number = $('#reorder_number').val();
                
                if (!deliveryId) {
                    alert('Хүргэлтийн ID олдсонгүй');
                    return;
                }
                ...
                const reOrderUrl = '{{ route('reorder-delivery') }}';
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        console.log("Re-ordering delivery...");
                    }
                }).done(function(result) {
                    if (result.success) {
                        alert('Амжилттай! Хүргэлт дахин үүсгэгдлээ');
                        $('#reorderModal').attr('style', 'display:none');
                        // Reload the table with current filter values
                        var currentStatus = $('#filterByStatus').val();
                        var currentRegion = $('#filterByBus').val();
                        var currentDriver = $('#filterByDriver').val();
                        var currentCustomer = $('#filterByCustomer').val();
                        var currentMerchant = $('#filterByCustomerMerchant').val();
                        var currentType = $('#filterByCustomerType').val();
                        var currentStartDate = $('#start_date').val();
                        var currentEndDate = $('#end_date').val();
                        loadDeliveryDataTable(currentStatus, currentRegion, currentDriver, currentCustomer, not_2, not_1, not_6, not_10,
                            not_100, currentMerchant, currentType, estimated, currentStartDate, currentEndDate);
                    } else {
                        alert('Алдаа гарлаа: ' + (result.message || 'Тодорхойгүй алдаа'));
                    }
                }).fail(function(xhr) {
                    var errorMsg = 'Алдаа гарлаа';

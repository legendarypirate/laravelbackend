                    ...
                    <div class="card-body table-responsive p-0" style="height: 300px;">
                        <table class="table table-head-fixed text-nowrap">
                            ...
                            <tbody>
                                ...
                                @foreach ($address as $regions)
                                    <tr>
                                        <td>{{ ++$i }}</td>
                                        <td>{{ $regions->title }}</td>
                                        <td>{{ $regions->description }}</td>
                                        <td><img src="/{{ $regions->sent_by_image }}" width=50></td>
                                        <td>{{ $regions->sent_by_name }}</td>
                                        <td>{{ $regions->phone }}</td>
                                        <td>{{ $regions->resolved_description }}</td>
                                        @if (auth()->user()->role != 'customer')
                                            <td>
                                                <button class="btn btn-warning showModalBtn"
                                                    data-feed-id="{{ $regions->id }}"
                                                    style="color:black;">Танилцсан</button>
                                            </td>
                                        @endif
                                    </tr>
                                @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 class="modal-title">Шийдвэрлэх</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>

                <!-- Modal body -->
                <div class="modal-body">
                    <input type="text" id="inputField" class="form-control" name="resolved_description"
                        placeholder="Агуулга">
                </div>

                <input value="" name="feedId" type="hidden" id="fid">
                <!-- Modal footer -->
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $(".showModalBtn").click(function() {
                // Get the data-feed-id attribute value for the clicked button
                var feedId = $(this).data("feed-id");

                // Set the value of the hidden input field with the retrieved feedId
                $("#fid").val(feedId);
            ...
            // You can also handle the submit button click here
            $("#submitBtn").click(function() {
                var inputValue = $("#inputField").val();
                // Do something with the input value, for example, submit it to a server or perform some action

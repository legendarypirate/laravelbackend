...
class FeedbackController extends Controller
{
    ...
    public function create(Request $request){
        ...
        $phone=Phone::where('userid',Auth::user()->id)->get();
        $data='';
        ...
        $phone->sent_by_id = Auth::user()->id;
        $phone->sent_by_image = Auth::user()->image;
        $phone->sent_by_name = Auth::user()->name;
        $phone->phone = $data;

        $phone->save();

        return redirect('/profile')->with('message','Амжилттай хадгалагдлаа');
    }
    
    public function resolve(Request $request){
        
        $phone = Feedback::find($request->feedId);
        $phone->resolved_description = $request-> resolved_description;
        $phone->save();

        return redirect('/feedback/list')->with('message','Амжилттай хадгалагдлаа');
    }
    
    public function list(){
        if(Auth::user()->role=='customer'){
            $address=Feedback::where('sent_by_id',Auth::user()->id)->get();
        } else {
            $address=Feedback::all();

        }
        return view('admin.feedback.list',compact('address'));
    }
